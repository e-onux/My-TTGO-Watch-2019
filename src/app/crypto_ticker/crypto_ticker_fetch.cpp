...
int crypto_ticker_fetch_price( crypto_ticker_config_t *crypto_ticker_config, crypto_ticker_widget_data_t *crypto_ticker_widget_data ) {
    char url[512]="";
    char currency_pair[10] = "";
    snprintf(currency_pair,sizeof(currency_pair),"XXBTZ%S", crypto_ticker_config->symbol);
    int httpcode = -1;

    
    snprintf( url, sizeof( url ), "https://api.kraken.com/0/public/Ticker?pair=%s", currency_pair);
    ...
    strcpy( crypto_ticker_widget_data->price, doc["result"][currency_pair]["a"][0] );
